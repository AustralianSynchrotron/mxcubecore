...
class ISPyBRestClient(HardwareObject):
    ...
    def authenticate(self, user, password):
        ...
        """
        auth_url = f"{self.__rest_root}/v1/authenticate"

        try:
            dict_data = {"user": user, "password": password}
            logging.getLogger("HWR").debug(f"LOGIN DETAILS: {dict_data}")
            data_json = json.dumps(dict_data)

            response = post(auth_url, data=data_json)
    ...
    def dc_link(self, did):
        ...
        if self.__rest_root is not None and did:
            # path = "mx/#/mx/proposal/{pcode}{pnumber}/datacollection/datacollectionid/{did}/main"
            path = "{pcode}{pnumber}/datacollection/datacollectionid/{did}/main"
            path = path.format(
            ...
            url = urljoin(self.__rest_root, path)
    ...
    def get_dc(self, dc_id):
        ...
        logging.getLogger("HWR").debug(f"URL: {url}")
        try:
            _response = json.loads(get(url).text)
            logging.getLogger(f"RESPONSE: {_response}")
    ...
    def get_proposals_by_user(self, user_name):
        ...
        if self.__rest_token:
            try:
                url = "{rest_root}/proposal/{username}/list"
                url = url.format(
                    rest_root=self.__rest_root,
